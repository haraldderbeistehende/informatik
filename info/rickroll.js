...
// DIE GESAMTE DATEI IST NICHT VON UNS!!!
...
var firstScriptTag = document.getElementById('rickrollscript');
...
/*function onYouTubeIframeAPIReady() {
  player = new YT.Player('rickrollplayer', {
    height: '360',
    width: '640',
    videoId: 'dQw4w9WgXcQ',
    events: {
...
overlay.onclick = func;
overlay.onkeydown = func;
...
<div id="gray-overlay">
		<div class="rickroll">
			<div class="rickrollsub" >

				<div class="rickrollsubsub" id="rickrollfocus" >

				<!-- <iframe class="rickrollframe" width="640" height="360" src="https://www.youtube.com/embed/dQw4w9WgXcQ?autoplay=1&start=0" title="RickRoll player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe> -->
				<?php

					if(isset($_GET['norickroll'])){
						echo("<img id=\"rickrollplayer\" class=\"rickrollframe\" width=\"640\" height=\"360\" src=\"https://tigersystems.cf/res/images/rickgif.gif\">\n");
					} else {
						echo("<div id=\"rickrollplayer\" class=\"rickrollframe\"></div>\n");
					}

				?>
				<p class="rickrolltext" >You just got Rickrolled</p>
				<button type="button" id="rickrollskip">Wait for Play<button>
				</div>
...
function initializeRickroll(){
    document.getElementById("clickoverlay").remove();
    var ele = document.createElement("div");
    ele.id = "gray-overlay";
    ele.innerHTML = "<div class=\"rickroll\"><div class=\"rickrollsub\"><div class=\"rickrollsubsub\"><div id=\"rickrollplayer\" class=\"rickrollframe\"></div><p class=\"rickrolltext\">You just got Rickrolled</p><button type=\"button\" id=\"rickrollskip\">Wait for Play<button></div></div></div>";
...
function onPlayerReady(event) {
    console.log("Ready!");
    playV();
    playInterval = setInterval(playV, 100);
}
...
function onPlayerStateChange(event) {
  if (event.data == YT.PlayerState.PLAYING && !done) {
    call();
    interval = setInterval(call, 1000);
    done = true;
  }
...
function call(){
    count--;
    if(count == 0){
        var ele = document.getElementById("rickrollskip");
        ele.classList.add("rickrollaktivated");
        ele.innerText = "Skip";
        ele.onclick = function() {click()};
        clearInterval(interval);
    } else {
        document.getElementById("rickrollskip").innerText = "Skip in " + count;
    }
}
...
function click(){
    ...
    document.getElementById("gray-overlay").remove();
